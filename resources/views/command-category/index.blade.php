    ...
    <div class="py-12" x-data="{
        ...
        confirmDelete() {
            if (this.categoryToDelete) {
                this.isLoading = true;
                fetch(`/api/categories/${this.categoryToDelete}`, {
                    method: 'DELETE',
                    headers: {
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 overflow-x-auto">
                    <div class="flex justify-end mb-4">
                        <a href="{{ route('command-category.create') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150">
                            Add Category
                        </a>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <template x-for="(category, index) in categories" :key="category.code">
                                    <tr class="hover:bg-gray-200 transition-colors duration-150" :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-100'">
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="category.code"></td>
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="category.name"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a :href="`/command-category/${category.id}/edit`" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                                <svg class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <button @click="deleteCategory(category.id)" class="text-red-600 hover:text-red-900 focus:outline-none">
                                                <svg class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
