<x-app-layout>
    ...
    <div class="py-12" x-data="{
        ...
        fetchData() {
            ...
            fetch(`/api/device-modules?${params.toString()}`)
        ...
        confirmDelete() {
            if (this.moduleToDelete) {
                this.isLoading = true;
                fetch(`/api/device-modules/${this.moduleToDelete}`, {
                    method: 'DELETE',
                    headers: {
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 overflow-x-auto">
                    <div class="flex justify-end mb-4">
                        <a href="{{ route('device-module.create') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150">
                            Add Device Module
                        </a>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-red-600">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-red-700 cursor-pointer hover:bg-red-700" @click="sortBy('tag_code')">
                                        <div class="flex items-center justify-between">
                                            Tag Code
                                            <template x-if="sortField === 'tag_code'">
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-red-700 cursor-pointer hover:bg-red-700" @click="sortBy('description')">
                                        <div class="flex items-center justify-between">
                                            Description
                                ...
                                <tr>
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.tag_code" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                    ...
                                    <th class="px-6 py-2 bg-white border-b border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.description" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-gray-200"></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                <template x-for="(module, index) in modules" :key="module.tag_code">
                                    <tr class="hover:bg-gray-200 transition-colors duration-150" :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-100'">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="module.tag_code"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="module.device_module"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900" x-text="module.description"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <a :href="`/device-module/${module.id}/edit`" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                                <svg class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <button @click="deleteModule(module.id)" class="text-red-600 hover:text-red-900 focus:outline-none">
                                                <svg class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    Are you sure you want to delete this device module? This action cannot be undone.
                                </p>
                            </div>
                        </div>
                    </div>
