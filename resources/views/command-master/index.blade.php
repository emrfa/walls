<x-app-layout>
    ...
    <div class="py-12" x-data="{
        ...
        init() {
            ...
            // Watch for changes in filters to reset to page 1 and fetch
            this.$watch('filters', () => {
                this.page = 1;
                this.fetchData();
            });
        ...
        deleteCommand(id) {
            this.commandToDelete = id;
            this.showDeleteModal = true;
        },

        confirmDelete() {
            if (this.commandToDelete) {
                this.isLoading = true;
                fetch(`/api/commands/${this.commandToDelete}`, {
                    method: 'DELETE',
                    headers: {
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 overflow-x-auto">
                    <div class="flex justify-end mb-4">
                        <a href="{{ route('command-master.create') }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700 active:bg-red-900 focus:outline-none focus:border-red-900 focus:ring ring-red-300 disabled:opacity-25 transition ease-in-out duration-150">
                            Add Command
                        </a>
                    </div>
                    ...
                    <div class="relative">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-red-600">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-gray-700 cursor-pointer hover:bg-red-700" @click="sortBy('device_module')">
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-red-700 cursor-pointer hover:bg-red-700" @click="sortBy('code')">
                                        <div class="flex items-center justify-between">
                                            Code
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-red-700 cursor-pointer hover:bg-red-700" @click="sortBy('set_reset')">
                                        <div class="flex items-center justify-between">
                                            Set/Reset
                                            <template x-if="sortField === 'set_reset'">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'" /></svg>
                                            </template>
                                        </div>
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-red-700 cursor-pointer hover:bg-red-700" @click="sortBy('opposite')">
                                        <div class="flex items-center justify-between">
                                            Opposite
                                            <template x-if="sortField === 'opposite'">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'" /></svg>
                                            </template>
                                        </div>
                                    </th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-bold text-white uppercase tracking-wider border-r border-red-700 cursor-pointer hover:bg-red-700" @click="sortBy('category')">
                                        <div class="flex items-center justify-between">
                                            Category
                                ...
                                <tr>
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.device_module" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.code" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                    ...
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.set_reset" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-r border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.opposite" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-gray-200">
                                        <input type="text" x-model.debounce.500ms="filters.category" class="block w-full text-xs rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Search...">
                                    </th>
                                    <th class="px-6 py-2 bg-white border-b border-gray-200"></th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                <template x-for="(command, index) in commands" :key="command.code">
                                    <tr class="hover:bg-gray-200 transition-colors duration-150" :class="index % 2 === 0 ? 'bg-white' : 'bg-gray-100'">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="command.code"></td>
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="command.command"></td>
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="command.set_reset"></td>
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="command.opposite"></td>
                                        <td class="px-6 py-4 text-sm text-gray-900" x-text="command.category"></td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">

                                            <a :href="`/command-master/${command.id}/edit`" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                                <svg class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                                </svg>
                                            </a>
                                            <button @click="deleteCommand(command.id)" class="text-red-600 hover:text-red-900 focus:outline-none">
                                                <svg class="h-5 w-5 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
